...
from qgis_epanet_integrator.elements import (
    DEFAULT_OPTIONS,
    DemandType,
    EnumWithName,
    Field,
...
from qgis_epanet_integrator.feature_reader import ReadFeatureError, read
from qgis_epanet_integrator.feature_writer import get_qgs_fields_from_options, write
from qgis_epanet_integrator.gusnet_processing.common import CommonProcessingBase
from qgis_epanet_integrator.i18n import tr
from qgis_epanet_integrator.inpfile_writer import write_inp_file
from qgis_epanet_integrator.network import Network
from qgis_epanet_integrator.pattern_curve import Pattern
from qgis_epanet_integrator.profiler import profile
from qgis_epanet_integrator.settings import ProjectSettings, SettingKey
from qgis_epanet_integrator.statistics import ModelStatistics
from qgis_epanet_integrator.style import style
from qgis_epanet_integrator.units import SpecificUnitNames
from qgis_epanet_integrator.verify_model import VerificationError, verify_model
from qgis_epanet_integrator.wntr_wrapper import EpanetError, WntrError, WntrWrapper
...
'links' (pipes, valves, pumps).
    ...
    def icon(self):
        return QIcon("qgis_epanet_integrator:run.svg")
    ...
    def processAlgorithm(  # noqa N802
        ...
        with logger_to_feedback("qgis_epanet_integrator", feedback):
            ...
            wntr_output_prefix = temp_file_dir / "wntr_run"
            ...
            # Run exactly the INP generated by Gusnet through WNTR's
            # EpanetSimulator.  This keeps the existing Gusnet model/UI,
            # while using the same EPANET execution path as QEV.
            with profile(tr("Run EPANET simulation through WNTR"), 50, feedback):
                try:
                    import wntr
                except ImportError as e:
                    raise QgsProcessingException(
                        tr(
                            "WNTR is not installed in the Python environment used by QGIS. "
                            "Install it with python-qgis.bat -m pip install --user wntr."
                        )
                    ) from e

                try:
                    wn = wntr.network.WaterNetworkModel(str(input_file))
                    wntr_model = WntrWrapper(wn)
                    wntr_model.run(output_file_prefix=str(wntr_output_prefix))
                    result_attributes = wntr_model.get_results(model_options.flow_units)
                except (EpanetError, WntrError, ValueError) as e:
                    raise QgsProcessingException(str(e)) from e
                except Exception as e:
                    raise QgsProcessingException(
                        tr("WNTR simulation failed: {exception}").format(exception=e)
                    ) from e
